...
export default new Event("messageDelete", async (message) => {
  ...
  const logChannel = message.guild.channels.cache.get(
    guildData.logs.channelID
  ) as TextChannel;
  let rolesWithPerms: Role[] = [];

  message.guild.roles.cache.forEach((role) => {
    if (role.permissions.has("ManageMessages") && !role.managed)
      rolesWithPerms.push(role);
  });
    ...
    if (message.content) {
      ...
      if (message.author === entry.executor) {
        deleteEmbed.setDescription(
          "The author deleted his message for whatever reason."
        );
      } else {
        deleteEmbed
          .setDescription(
            `Someone with the \`MANAGE_MESSAGES\` permission (probably one of these: ${rolesWithPerms
              .map((role) => `${role}`)
              .join(", ")}) deleted the message of a member in this server.`
          )
          .addFields({
            name: "Deleted by",
