import { ChannelType, CommandInteractionOptionResolver, GuildMember, PermissionResolvable } from "discord.js"
import { client } from ".."
    ...
    if (interaction.isButton()) {
        ...
        if (interaction.customId.includes("role")) {
            ...
            if (interaction.guild.roles.cache.find(role => role.id === roleID)) {
                ...
                if (!interaction.guild.members.me.permissions.has("ManageRoles")) return interaction.reply({
            ...
            if (guildData.tickets.enabled) {
                ...
                if (!interaction.guild.members.me.permissions.has("ManageMessages")) return interaction.reply({
                    content: `I'm missing the permission to manage messages in this guild. Contact the server administrator.`,
                    ephemeral: true
                })
                if (!interaction.guild.members.me.permissions.has("AddReactions")) return interaction.reply({
                    content: `I'm missing the permission to add reactions in this guild. Contact the server administrator.`,
                    ephemeral: true
                })

                const ticketCategory = guild.channels.cache.get(guildData.tickets.categoryID)
                const ticketChannel = guild.channels.cache.get(guildData.tickets.channelID)
                if (ticketCategory && ticketChannel) {
                    if (ticketCategory.type === ChannelType.GuildCategory && ticketChannel.type === ChannelType.GuildText) {
