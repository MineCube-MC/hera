...
export default new Event("guildMemberAdd", async (member) => {
  ...
  if (guildData.welcome.enabled) {
    ...
    let avatarURL = member.user.displayAvatarURL({
      extension: "png",
      forceStatic: true,
      size: av.size as ImageSize,
    });

    const canvas = Canvas.createCanvas(dim.width, dim.height);
    const ctx = canvas.getContext("2d");

    const backimg = await Canvas.loadImage(
      path.join(process.cwd() + `/assets/cards/card-${number}.png`)
    );
    ...
    const welcomeChannel = member.guild.channels.cache.find(
      (ch) => ch.id === guildData.welcome.channelID
    ) as TextChannel;
    if (welcomeChannel) {
      let message = (guildData.welcome.text as string)
